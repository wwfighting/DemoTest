 ...
 */
public class DetailColAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public DetailColAdapter(Context context, GoodsDetailInfo infoList) {
    ...
    public int getItemCount() {
        return mInfoList.getData().getColors().size();
    }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ImgClassViewHolder){
            Glide.with(mContext)
                    .load(Constants.BASE_IMG_URL + mInfoList.getData().getColors().get(position).getCpath())
                    .crossFade()
