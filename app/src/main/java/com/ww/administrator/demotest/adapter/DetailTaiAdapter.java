 ...
 */
public class DetailTaiAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public DetailTaiAdapter(Context context, GoodsDetailInfo infoList) {
    ...
    public int getItemCount() {
        return mInfoList.getData().getTaimian().size();
    }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ImgClassViewHolder){
            Glide.with(mContext)
                    .load(Constants.BASE_IMG_URL + mInfoList.getData().getTaimian().get(position).getTpath())
                    .crossFade()
