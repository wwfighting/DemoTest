...
import com.ww.administrator.demotest.util.AnimUtil;
import com.ww.administrator.demotest.util.ColorUtil;
    ...
    public void onStopNestedScroll(View child) {
        if (Math.abs(totalDrag) >= dragDismissDistance) {
            ...
            animate()
                    ...
                    .scaleY(1f)
                    .setDuration(200L)
                    .setInterpolator(AnimUtil.getFastOutSlowInInterpolator(getContext()))
                    .setListener(null)
                    .start();
            totalDrag = 0;
    ...
    public static abstract class SystemChromeFader implements ElasticDragDismissListener {
        ...
        public void onDrag(float elasticOffset, float elasticOffsetPixels,
            ...
            if (elasticOffsetPixels < 0) {
                ...
                window.setNavigationBarColor(ColorUtil.modifyAlpha(window.getNavigationBarColor(),
                        1f - rawOffset));
            } else if (elasticOffsetPixels == 0) {
                // reset
                window.setStatusBarColor(ColorUtil.modifyAlpha(window.getStatusBarColor(), 1f));
                window.setNavigationBarColor(
                        ColorUtil.modifyAlpha(window.getNavigationBarColor(), 1f));
            } else {
                // dragging downward, fade the status bar in proportion
                window.setStatusBarColor(ColorUtil.modifyAlpha(window
